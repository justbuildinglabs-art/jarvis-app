...
import { makeImpulse } from "./audio";
import { CHARS_PER_SEC, DISMISS_RE, WAKE_EVENTS_URL } from "./constants";
import type {
  DeliverableListener,
  ListeningListener,
  LogListener,
...
// ---------------------------------------------------------------------------
// The client-side voice singleton — the browser half of the voice loop.
//
// Playback: speak(text) queues an utterance; each plays through an <audio>
// element wired into a Web Audio AnalyserNode, so getLevel() exposes the real
// RMS envelope the 3D core mouths. Autoplay policy means nothing plays until
// the first user gesture, so utterances queue and drain on unlock.
//
// Capture: startCapture()/finishCapture() record the mic (MediaRecorder),
// POST the clip to /api/voice, and speak whatever comes back. Holding Space
// is itself a gesture, so push-to-talk doubles as the unlock.
//
// Everything here is browser-only and stateful, which is why it is one class
// and one instance: two of these would mean two audio graphs, two WebSockets
// and doubled speech. The pure parts it used to carry — sanitising text,
// building the reverb impulse, the tunables — now live beside it in
// ./text.ts, ./audio.ts and ./constants.ts, where they can be read and tested
// without a DOM.
// ---------------------------------------------------------------------------
    ...
    fetch("/api/speak", { cache: "no-store" })
      .then(async (res) => {
        if (res.status === 503) {
          this.disabled = true;
          this.queue = [];
          this.log("err", "voice offline — voice-server on :4871 is down");
        } else if (res.ok) {
