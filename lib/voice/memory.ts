...
// ---------------------------------------------------------------------------
// Conversational memory — a file-backed ring of recent exchanges, so that
// "make it shorter" and "what about the second one" have something to refer
// to.
//
// JSONL in the vault rather than process RAM, for three reasons that all
// turned out to matter: it survives a Next dev restart mid-conversation, it
// can be read with `tail` when a follow-up resolves oddly, and it does not
// make conversation continuity depend on the voice-server being up.
//
// What gets surfaced is narrower than what gets stored. The file keeps the
// last MAX_KEEP exchanges; a prompt only sees the ones inside the active
// conversation window, because a reference to "that" from an hour ago is
// noise, not context.
// ---------------------------------------------------------------------------
