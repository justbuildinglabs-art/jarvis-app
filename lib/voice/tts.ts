...
// Engine: local Kokoro voice-server on :4871 (free, offline, no network lag).
...
export async function speak(text: string): Promise<SpeechStream> {
  ...
  throw new VoiceConfigError(
    "no TTS engine: the voice-server on :4871 is down (voice-server\\start-voice-server.cmd)"
  );
}
